...
void do_check(Cnn * self) {
	...
	if (!self->on_connected) croak("on_connected not initialized");
	if (!self->on_disconnect) croak("on_disconnect not initialized");
	if (!self->on_read) croak("on_read not initialized");
}
...
int main () {
	...
	adns_init (&self->ads, adns_if_noenv | adns_if_noerrprint | adns_if_noserverwarn | adns_if_noautosys, 0);
	
	ev_idle iw;
	//ev_idle_init
	
	int r = adns_submit (self->ads, "google.com", adns_r_addr, 0, (void *) self, &self->ads_query);
	//if (!ev_is_active (&iw))
	...
	r = adns_check (self->ads, &q, &a, &ctx);
	cwarn("r = %d (%s)",r,strerror(r));
	...
	while (r = adns_beforepoll (self->ads, fds, &nfd, &timeout, &tv_now)) {
		cwarn("before: %d",r);
	}
	...
	self->iaddr.sin_port        = htons( 33013 );
